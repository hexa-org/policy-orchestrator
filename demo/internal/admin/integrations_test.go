...
func (suite *IntegrationsSuite) TestListIntegrations_templateRenders() {
	...
	assert.Contains(suite.T(), string(body), "Google IAP")
	assert.Contains(suite.T(), string(body), "Azure")
	assert.Contains(suite.T(), string(body), "AWS Cognito")
	assert.Contains(suite.T(), string(body), "AWS AVP")
...
func (suite *IntegrationsSuite) TestCreateIntegration_withOPA() {
	...
	for _, tt := range tests {
		suite.Run(tt.name, func() {
			...
			assert.Equal(suite.T(), "opa-bundle-url/:open-policy-agent", suite.client.Name)
		})
	}
}
